...
bool InitList(SQList& list) {
    ...
    if (!list.elem) return false;       // 创建失败，返回值0
...
void DestoryList(SQList* list) {
    ...
    if (list->elem) delete[] list->elem;  // 指针判空,释放储存空间
    list->elem = NULL;                    // 指针赋空
}
...
int LocateElem(SQList* list, ElemType e) {
    for (int i = 0; i <= list->length - 1; i++)
        if (list->elem[i] == e) return i + 1;  // 查找成功，返回为序号
    return 0;                                  // 查找失败，返回0
}
// 4.2 查找 按值查找
int LocateElem(SQList& list, ElemType e) {
    int i = 0;
    while (i < list.length && e != list.elem[i]) i++;
    if (i < list.length) return i + 1;  // 查找成功，返回为序号
    return 0;                           // 查找失败，返回0
}
// 4.3 查找 按值查找
// 返回所在表中地址
...
bool ListInsert(SQList& list, int i, ElemType e) {  // i是位序
    if (MAXSIZE == list.length) return false;       // 判定储存是否已满
    if (i >= MAXSIZE) return false;  // i值大于容量，不合法
    if (i < 1 || i > list.length + 1) return false;  // 判定i值是否合法
...
void push_front(SQList* L, ElemType item) {
    ...
    for (int i = L->length - 1; i >= 0; i--) L->elem[i + 1] = L->elem[i];
    L->elem[0] = item;
    L->length++;
}
...
bool ListDelete(SQList& list, int i, ElemType& e) {  // i是位序
    if (i > list.length || i < 1) return false;      // 判定i值是否合法
...
void pop_front(SQList& L) {
    ...
    if (L.length >= 1) {
        ...
        for (int i = 1; i <= L.length; i++) L.elem[i - 1] = L.elem[i];
    }
    L.length--;  // 表长减1
}
// 6.3 尾删
...
void MergeList(SQList la, SQList lb, SQList& lc) {
    ...
    delete[] lc.elem;  // 释放之前申请内存空间
    lc.length = la.length + lb.length;
    lc.elem = new ElemType[lc.length];
    lc.length = 0;  // lc表重新新建完成，长度复位
    pc = lc.elem;
    ...
    while (pa <= pa_last) *pc++ = *pa++, lc.length++;  // 拷贝剩余值
    while (pb <= pb_last) *pc++ = *pb++, lc.length++;  // 拷贝剩余值
}
    ...
    while (select) {
        ...
        switch (select) {
            case 1:
                ...
                while (scanf("%d", &item), item != -1) push_back(&mylist, item);
                break;
            case 2:
                printf("please input insert data(endup with -1):>\n");
            ...
            case 15: {
                ...
                while (scanf("%d", &item), item != -1) push_back(&la, item);
                printf("table Lb:insert data(endup with -1):>\n");
                while (scanf("%d", &item), item != -1) push_back(&lb, item);
