	    	...
	    	case 1:
				{
					...
					printf("请输入要插入的数据（-1,0结束）:>\n");
		      		while (scanf("%f%d", &item.coef, &item.expn), item.coef != -1.0f) push_back<polyno, ElemType>(&table, item);					
				} break;
	    	case 2:
	    		{
	    			ElemType item;
	   				printf("请输入要插入的数据（-1,0结束）:>\n");
	      			while (scanf("%f%d", &item.coef, &item.expn), item.coef != -1.0f) push_front<polyno, ElemType>(&table, item); 			
				} break;
	      	case 3:
			  	PolynoShow(&table);
				break;	
			...
			case 6:
				{
					...
					printf("请给Pa输入要插入的数据(x, n)（-1,0结束）:>\n");
	      			while ( scanf("%f %d", &item.coef, &item.expn), item.coef != -1.0f ) push_back(&pa, item);
					printf("请给Pb输入要插入的数据(x, n)（-1,0结束）:>\n");
	      			while ( scanf("%f %d", &item.coef, &item.expn), item.coef != -1.0f ) push_back(&pb, item);
					*/
			...
			case 8:
				{
					...
					if (isfileNull(FILE_Import)) {
						...
						//clear stdin buffer 
						//fseek(stdin, 0, SEEK_SET);						/* 清空输入缓冲区 */
						//rewind(stdin);
						//fflush(stdin);
						int ch = getchar();									// 读取整数 
						int cnt = 1;										// 缓存区存在\n 
						while (ch != '$') {
							if (ch == '\n' && cnt == 1) cnt = 0, ch = getchar(); // 如果是换行取出来 
							fputc(ch, fp);
							ch = getchar();
						}
						fclose(fp);
...
bool InitialPolyno(T *ptab) {
	...
	if (!ptab->elem) return false;			// 分配失败返回0 
...
void pop_front(T* ptab) {
	if (ptab->length == 0) return;	// if seqlist is empty
	if (ptab->length >= 1) {
...
void pop_back(T* ptab) {
	if (ptab->length == 0) return;
	if (ptab->length >= 1) ptab->length--;
}

// 3.0 返回表长 
template<typename T>
int GetLength(T *ptab) {
	if (ptab->elem) return ptab->length;
	else return -1;
}
...
void SubPolyno(T* Pa, T* Pb, T* Res) {
	...
	*/
	...
	tab.length = 0;
	for (int i = 0; i < Pb->length; i++) Pb->elem[i].coef *= -1;
	AddPolyno(Pa, Pb, &tab);
	Res->length = tab.length;
	Res->elem = tab.elem;
}
...
void MulPolyno(T* Pa, T* Pb, T* merge) {
	...
	for (int i = 0; i < Pa->length; i++) 
		for (int j = 0; j < Pb->length; j++) {
			ptab.elem[ptab.length].coef = Pa->elem[i].coef * Pb->elem[j].coef;	// 系数相乘 
			ptab.elem[ptab.length].expn = Pa->elem[i].expn + Pb->elem[j].expn;	// 指数相加 
			ptab.length++; // lens + 1
		}
	printf("mul result: \n");
	PolynoShow(&ptab);	
	...
	for (int i = 0; i < ptab.length; flag ? i = index, flag = 0 : i++)
		for (int j = i + 1; j < ptab.length; j++) {
			if (ptab.elem[i].expn == ptab.elem[j].expn && (visit[i] == 0 && visit[j] ==0)) {
			...
			} else if (ptab.elem[i].expn < ptab.elem[j].expn && (visit[i] == 0 && visit[j] == 0)) {
				if (j == ptab.length - 1) {
					merge->elem[merge->length].expn = ptab.elem[i].expn;
