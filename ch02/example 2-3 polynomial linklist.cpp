  ...
  while (p1 && p2) {
    if (p1->elem.expn == p2->elem.expn) {
      ...
      if (coef == 0) { // result is 0，move ponit:p1, p2
        r = p1;
        p1 = p1->next;
        delete r; // delete p1 node
        r = p2;
        p2 = p2->next;
        delete r; // delete p2 node
      } else {
  ...
  */
  ...
  for (tail = Pd, prior = NULL; p = Pc->next;) {
    ...
    while (fast) { // 逆序排列(也可改大于号即为升序排序)
      if (max->elem.expn < fast->elem.expn)
        flag = 0, prior = slow, max = fast;
      slow = fast; // 快慢指针法
      fast = fast->next;
    }
    if (flag)
      Pc->next = max->next; // max node 等于头结点
    else {
      // max node 等于其他结点
      prior->next = max->next;
    }
