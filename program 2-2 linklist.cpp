...
int Delete_val(LinkList &L, ElemType e) {
  ...
  LNode *p = L->next; // L链表快指针p
  LNode *s = L;       // L链表慢指针s
  LNode *q = list;    // 指针q迭代
  while (p) {
    if (e.elem != p->data.elem) {
      q->next = p;
      q = p;
      s->next = p->next; // 储存头尾结点
                         //			s = p->next;
      cout << q->data.elem << endl;
    }
    if (p->next == NULL)
      q->next = NULL; // 下个结点是链尾，q->next指向空
    if (e.elem == p->data.elem)
      s = p;     // 越过相等结点
    p = p->next; // 指针p迭代
  }
...
bool ListCreate_R(LinkList &L, int n) {
  ...
  rear = L;       // 尾指针指向L
                  //	for (int i = 0; i < n; i++) {
                  //		p = new LNode, p->data.elem = i+1;
                  //		p->next = NULL;
                  //		rear->next = p; 					//
  //rear->next结点指向p 		rear = p;
  //// rear指针移动(L指针一直不动)
  //	}
  for (int i = 0; i < n; i++) {
    p = new LNode, p->data.elem = i + 1;
...
int main(int argc, char **argv) {
  ...
  while (select) {
    ...
    case 7: {
      int sele = 0;
      printf("请选择:按值查找:>1 or 查找中点:>2 or 按倒数查找:>3 or "
             "查找最值:>4 \n");
      scanf("%d", &sele);
      ElemType *p;
      switch (sele) {
