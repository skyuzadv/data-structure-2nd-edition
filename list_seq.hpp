#ifndef __list_seq_hpp__
#define __list_seq_hpp__
...
class List_seq {
   ...
   public:
     ...
     * @return false
     */
    bool ListInsert(int index, ElemType elem) {    // i是位序
     ...
     * @param LS
     */
    void MergeList(List_seq<ElemType> LS) {
        int n = LS.list.length;
        ElemType item;
        for (std::size_t i = 0; i < n; i++) {
            GetElem(i, item);  // LS：获取元素
            if (this->LocateElem(item) == -1)
                ListInsert(++(this->list.length), item);
        }
    }
    /**
     * @brief 有序表合并
     ...
     * @param LSres 存储表
     */
    void MergeList(List_seq<ElemType> const& LSdes, List_seq<ElemType>& LSres) {
#ifndef ARRAY
